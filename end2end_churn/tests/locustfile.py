...
def on_test_stop(environment, **kwargs):
    ...
    if predict_stats and predict_stats.num_requests > 0:
        ...
        print(f"   {'✅' if p95_pass else '❌'} p95 < {p95_slo}ms: {p95:.0f}ms {'PASS' if p95_pass else 'FAIL'}")
        
        # SLO 2: p99 latency < 1000ms
        p99_slo = 1000  # ms
        p99_pass = p99 < p99_slo
        print(f"   {'✅' if p99_pass else '❌'} p99 < {p99_slo}ms: {p99:.0f}ms {'PASS' if p99_pass else 'FAIL'}")
        
        # SLO 3: Error rate < 1%
        error_rate_slo = 1.0  # %
        error_pass = error_rate < error_rate_slo
        print(f"   {'✅' if error_pass else '❌'} Error rate < {error_rate_slo}%: {error_rate:.2f}% {'PASS' if error_pass else 'FAIL'}")
